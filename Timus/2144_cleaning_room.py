...
def all_boxes_ok(boxes):
	...
	non-decreasing order of height, False otherwise
	"""
	for box in boxes:
		if not box_ok(box):
			return False
...
def box_ok(box):
	"""
	box is a the list of action figure heights in a given box.
	
	Return True if the heights in box are in non-decreasing order.
	False otherwise
	"""
...
def boxes_endpoints(boxes):
	...
	the heights of the leftmost and rightmost action figure in a box.
	"""
	endpoints = []
	for box in boxes:
...
def all_endpoints_ok(endpoints):
	"""
	endpoints is a list, where each value if a list of two values:
